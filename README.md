...
「便捷功能 → 安装与代理」中，系统信息卡片下方提供「本应用新版本检测」。它读取本项目 GitHub Releases 最新正式版，按语义版本比较，不会降级，也不会安装预发布版。检测和下载使用「安装与代理」页面当前选择的代理（无需先保存）；后台接口未传代理参数时使用已保存的代理配置。

发现新版本后，Windows 打包版显示「下载并重启更新」。文件通过大小、Windows x64 PE 格式和 GitHub 提供的 SHA-256 校验后，隐藏更新助手等待当前程序退出，在原位置替换 EXE，并启动新版。新窗口和托盘启动成功后清理临时旧版；新版启动失败会尝试恢复旧版。配置、密钥和 backups 不参与替换。

如果网络、目录权限或更新助手受限，或当前以源码 / 非 Windows 方式运行，可点击「前往发布页」手动下载。替换阶段失败会自动打开发布页，并在程序目录留下 `update-error.log`；无法恢复时会保留临时目录中的 `previous.exe`，便于手动恢复。

发布新版本时，统一修改 `switcher/__init__.py` 的版本号，再重新构建并使用相同的 `vX.Y.Z` 发布标签。构建脚本自动将版本写入 EXE 属性。发布正式版并上传唯一的 `ChatGPT.Switch.exe`（也兼容 `ChatGPT Switch.exe` 或带 Windows x64 后缀的名称）；自动更新依赖 GitHub 资产返回的 SHA-256 digest。

「设置」最后一项为「关于我们」，包含当前版本、项目介绍、开源许可、GitHub 项目主页和问题反馈入口。
